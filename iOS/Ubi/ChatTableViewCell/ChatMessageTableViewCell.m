...
- (void)configureCellWithMessage:(QBChatAbstractMessage *)message andRecipientFullName:(NSString *)recipientFullName
{    
    ...
    if ([LocalStorageService shared].currentUser.ID == message.senderID) {
        ...
        self.dateLabel.text = [NSString stringWithFormat:@"%@, %@", [[LocalStorageService shared].currentUser fullName], time];
		...
		self.dateLabel.text = [NSString stringWithFormat:@"%@, %@", (recipientFullName ? : [NSString stringWithFormat:@"%lu", (unsigned long)message.senderID]), time];
    }
}
