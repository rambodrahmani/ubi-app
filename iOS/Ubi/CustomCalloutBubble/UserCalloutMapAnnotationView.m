...
@interface UserCalloutMapAnnotationView()
...
@implementation UserCalloutMapAnnotationView
...
- (void)initProfilePicView:(UbiUser *)newUbiUser
{
	...
	[_profilePicView sd_setImageWithURL:newUbiUser.profile_pic
...
- (int)initLabels:(UbiUser *)newUbiUser :(CGSize)size
{
    ...
    NSString * Nome = [[NSString alloc] initWithFormat:@"%@ %@", newUbiUser.name, newUbiUser.surname];
    ...
    NSString * Bio = [[NSString alloc] initWithFormat:@"%@", newUbiUser.bio];
    int bioMultiplier = 1;
    if (Bio.length > 30) {
        bioMultiplier = 2;
    }
    ...
    NSString * lastPost = [[NSString alloc] initWithFormat:@"%@", newUbiUser.last_status_text];
