//
//  EventCalloutMapAnnotationView.m
//  Ubi
//
...
@interface EventCalloutMapAnnotationView()
...
@implementation EventCalloutMapAnnotationView
...
{
    _profilePicView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"userIcon"]];
    CGSize newSize = CGSizeMake(90, 90);
    _profilePicView.frame = CGRectMake(8, 8, newSize.width, newSize.height);
    
	[_profilePicView sd_setImageWithURL:newUbiEvent.event_picture_url
					   placeholderImage:[UIImage imageNamed:@""]];
	
    [_profilePicView setContentMode:UIViewContentModeScaleToFill];
    _profilePicView.layer.cornerRadius = (_profilePicView.frame.size.width/2);
    _profilePicView.clipsToBounds = YES;
    
    [_contentView addSubview:_profilePicView];
}
...
- (int)initLabels:(UbiEvent *)newUbiEvent :(CGSize)size
{
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
        labelsWidth = 500 - _profilePicView.frame.size.width - 36;
    }
    else {
        labelsWidth = size.width - _profilePicView.frame.size.width - 36;
    }
    
    NSString * Nome = [[NSString alloc] initWithFormat:@"%@", newUbiEvent.event_name];
    UILabel * lblNome =  [[UILabel alloc] initWithFrame:CGRectMake(8 + _profilePicView.frame.size.width + 8, 8, labelsWidth, 21)];
    [lblNome setTextColor:[UIColor whiteColor]];
    lblNome.text = Nome;
    [_contentView addSubview:lblNome];
    
    NSString * Bio = [[NSString alloc] initWithFormat:@"%@", newUbiEvent.event_description];
    int bioMultiplier = 1;
    if (Bio.length > 30) {
        bioMultiplier = 2;
    }
    UILabel * lblBio =  [[UILabel alloc] initWithFrame:CGRectMake(8 + _profilePicView.frame.size.width + 8, lblNome.frame.origin.y + 23, labelsWidth, bioMultiplier * 21)];
    lblBio.numberOfLines = bioMultiplier;
    [lblBio setTextColor:[UIColor whiteColor]];
    lblBio.text = Bio;
    [_contentView addSubview:lblBio];
    
    NSString * lastPost = [[NSString alloc] initWithFormat:@"%@", newUbiEvent.event_start_date];
    int postMultiplier = 1;
    if (lastPost.length > 30) {
        postMultiplier = 2;
    }
    UILabel * lbllastPost =  [[UILabel alloc] initWithFrame:CGRectMake(8 + _profilePicView.frame.size.width + 8, lblBio.frame.origin.y + 23 + (bioMultiplier == 1 ? 0 : 21), labelsWidth, postMultiplier * 21)];
    lbllastPost.numberOfLines = postMultiplier;
    [lbllastPost setTextColor:[UIColor whiteColor]];
    lbllastPost.text = lastPost;
    [_contentView addSubview:lbllastPost];
    
    int coordY = 0;
    if (bioMultiplier + postMultiplier <= 3) {
        coordY = _profilePicView.frame.size.height + 18;
    }
    else {
        _contentHeight = 175.0f;
        coordY = _profilePicView.frame.size.height + 35;
    }
...
- (void)initSocialLogo:(int)coordY :(NSString *)profileURL
{
    ...
    UIImage * Logo;
    NSRange rangeFB = [profileURL rangeOfString:@"facebook"];
    NSRange rangeTW = [profileURL rangeOfString:@"twitter"];
    NSRange rangeGP = [profileURL rangeOfString:@"google"];
    if (rangeFB.length != 0) {
        Logo = [UIImage imageNamed:@"fbLogoFull"];
    }
    else if (rangeTW.length != 0) {
        Logo = [UIImage imageNamed:@"twLogoFull"];
    }
    else if (rangeGP.length != 0) {
        Logo = [UIImage imageNamed:@"gpLogoFull"];
    }
    ...
    _SocialLogoView = [[UIImageView alloc] initWithImage:Logo];
    _SocialLogoView.frame = CGRectMake(8, coordY, newSize.width, newSize.height);
    
    [_contentView addSubview:_SocialLogoView];
}

- (void)initAddFriend:(int)coordY
{
    UIImage * addFriendLogo = [UIImage imageNamed:@"addFriend.png"];
    CGSize newSize = CGSizeMake(40, 40);
    UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);
    [addFriendLogo drawInRect:CGRectMake(0, 0, newSize.width, newSize.height)];
    addFriendLogo = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    _addFriendLogoView = [[UIImageView alloc] initWithImage:addFriendLogo];
    
    _addFriendLogoView.frame = CGRectMake(8 + _SocialLogoView.frame.size.width + 8, coordY, newSize.width, newSize.height);
    
    [_contentView addSubview:_addFriendLogoView];
}

- (void)initSendMessage:(int)coordY
{
    UIImage * sendMessageLogo = [UIImage imageNamed:@"sendMessage.png"];
    CGSize newSize = CGSizeMake(40, 40);
    UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);
    [sendMessageLogo drawInRect:CGRectMake(0, 0, newSize.width, newSize.height)];
    sendMessageLogo = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    _sendMessageLogoView = [[UIImageView alloc] initWithImage:sendMessageLogo];
    
    _sendMessageLogoView.frame = CGRectMake(8 + _addFriendLogoView.frame.size.width + 8 + _SocialLogoView.frame.size.width + 8, coordY, newSize.width, newSize.height);
    
    [_contentView addSubview:_sendMessageLogoView];
}

- (void)initBuzz:(int)coordY
{
    UIImage * buzzLogo = [UIImage imageNamed:@"buzz.png"];
    CGSize newSize = CGSizeMake(40, 40);
    UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);
    [buzzLogo drawInRect:CGRectMake(0, 0, newSize.width, newSize.height)];
    buzzLogo = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    _buzzLogoView = [[UIImageView alloc] initWithImage:buzzLogo];
    
    _buzzLogoView.frame = CGRectMake(8 + _addFriendLogoView.frame.size.width + 8 + _SocialLogoView.frame.size.width + 8 + _sendMessageLogoView.frame.size.width + 8, coordY, newSize.width, newSize.height);
    
    [_contentView addSubview:_buzzLogoView];
}

- (void)initLblDistance:(NSNumber *)distance :(CGSize)size
{
    UILabel * lblDistance =  [[UILabel alloc] initWithFrame:CGRectMake(size.width - 70, _SocialLogoView.frame.origin.y + 19, 80, 21)];
    [lblDistance setTextColor:[UIColor whiteColor]];
    lblDistance.text = [[NSString alloc] initWithFormat:@"%.0f%@", (distance.floatValue > 1000 ? distance.floatValue/1000 : distance.floatValue), (distance.floatValue > 1000 ? @"km" : @"m")];
    [_contentView addSubview:lblDistance];
}
...
- (void)prepareFrameSize
{
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
        frame.size = CGSizeMake(500, height);
    }
    else {
...
- (void)prepareOffset
{
	...
	self.centerOffset = CGPointMake(xOffset + 8, yOffset);
}
