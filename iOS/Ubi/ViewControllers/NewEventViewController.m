...
//  Ubi
//
//  Created by Rambod Rahmani on 22/12/14.
...
- (void)viewDidLoad {
	...
	_datePickerView.layer.borderWidth = 1.0;
	_datePickerView.layer.borderColor = [[UIColor colorWithRed:(34.0/255.0) green:(206.0/255.0) blue:(210.0/255.0) alpha:1] CGColor];
	...
	[[[UIAlertView alloc] initWithTitle:@"Eventi"
								message:@"Ricordati che puoi gestire i tuoi eventi in maniera più comoda e semplice dal pannello di controllo web: provalo su www.ubisocial.it."
							   delegate:self
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
	...
	if ([textView.text isEqualToString:@"Descrizione evento..."]) {
		textView.text = @"";
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
	if ([textView.text isEqualToString:@""]) {
		textView.text = @"Descrizione evento...";
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
	if (locationsTableViewOpen) {
		...
		[[[UIAlertView alloc] initWithTitle:@"Ricerca amici"
									message:@"La ricerca nella lista delle amicizie non è ancora stata abilitata."
								   delegate:self
						  cancelButtonTitle:@"OK"
						  otherButtonTitles:nil] show];
	}
...
- (IBAction)closeDatePicker:(id)sender {
	[_txtFieldStart setText:[NSString stringWithFormat:@"%@", _datePicker.date]];
	_datePickerView.hidden = YES;
	_datePicker.hidden = YES;
}

- (IBAction)closeDatePicker_sec:(id)sender {
	[_txtFieldEnd setText:[NSString stringWithFormat:@"%@", _datePicker.date]];
	_datePickerView.hidden = YES;
	_datePicker.hidden = YES;
}
...
- (void)showTableView:(BOOL)scrolling
{
	...
	[_txtFieldEventName resignFirstResponder];
	[_txtFieldStart resignFirstResponder];
	[_txtFieldEnd resignFirstResponder];
...
- (IBAction)sendNewPost:(id)sender
{
	...
	{
		if ( (_txtFieldEnd.text.length < 1) || (_txtFieldStart.text.length < 1) || (_txtFieldEventName.text.length < 1) || (selectedFriends.count == 0) || (selectedPlace.name.length < 1) || ([_txtViewPost.text isEqualToString:@"Descrizione evento..."]) || (_txtViewPost.text.length < 1) || (!_selectedImageView.image) ) {
			[[[UIAlertView alloc] initWithTitle:@"Errore"
										message:@"Devi inserire tutti i campi richiesti per poter creare un evento (Nome, Data Inzio, Data Fine, Descriizone, Location, Share, Tags e Media per l'evento)."
									   delegate:self
							  cancelButtonTitle:@"OK"
							  otherButtonTitles:nil] show];
			return;
		...
		if ([_txtViewPost.text isEqualToString:@"Descrizione evento..."]) {
			postContent = @"";
		}
		else {
		...
		NSString * mediaURL = [NSString stringWithFormat:@"%@/uploads/events/%@/%@", BASE_URL, currentUbiUser.db_id, fileName];
		if (!_selectedImageView.image) {
			mediaURL = @"NO_MEDIA";
		}
		...
		NSDictionary *params = @{@"place_google_id": ([place_name isEqualToString:@"NO_LOC"] ? place_name : selectedPlace.placeId),
								 ...
								 @"event_author_id": currentUbiUser.db_id,
								 @"event_name": _txtFieldEventName.text,
								 @"event_picture_url": mediaURL,
								 @"event_description": postContent,
								 @"event_start_date": _txtFieldStart.text,
								 @"event_end_date": _txtFieldEnd.text,
								 
								 @"event_date_utc_offset": timeZoneInMins};
		
		[[manager POST:@"/php/1.1/post_new_event.php" parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData)
		  {
			  if (_selectedImageView.image) {
			  ...
			  if ( !(range.length != 0) )
			  {
				  ...
				  [self showErrorMessage:[NSString stringWithFormat:@"Something went wrong. Please retry. Message: %@", operation.responseString]];
			  }
		  } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
			  [[[UIAlertView alloc] initWithTitle:@"Errore"
										  message:[NSString stringWithFormat:@"Si è verificato un errore durante il caricamento del media allegato all'evento. %@", error.description]
										 delegate:self
								cancelButtonTitle:@"OK"
								otherButtonTitles:nil] show];
		  }] start];
